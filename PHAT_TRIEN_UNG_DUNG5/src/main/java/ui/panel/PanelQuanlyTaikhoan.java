...
public class PanelQuanlyTaikhoan extends JPanel implements ActionListener, MouseListener {
	...
	private CustomJTextField tfPassword;
	private CustomJTextField tfUsername;
	...
	public PanelQuanlyTaikhoan() {
		...
		CustomJLabel lblTitle = new CustomJLabel("QUẢN LÝ TÀI KHOẢN ĐĂNG NHẬP HỆ THỐNG");
		...
		CustomJLabel lblPassword = new CustomJLabel("Mật khẩu");
		lblPassword.setBounds(420, 82, 119, 25);
		add(lblPassword);

		CustomJLabel lblUsername = new CustomJLabel("Tên đăng nhập");
		lblUsername.setBounds(10, 82, 163, 25);
		add(lblUsername);

		CustomJLabel lblMaNV = new CustomJLabel("Mã NV");
		lblMaNV.setBounds(10, 41, 97, 25);
		add(lblMaNV);

		tfPassword = new CustomJTextField("");
		tfPassword.setBounds(510, 82, 264, 25);
		add(tfPassword);

		tfUsername = new CustomJTextField(20);
		...
		CustomJScrollPane scrollPane = new CustomJScrollPane(table, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
...
//		JPanel panel = new JPanel();
	...
	public PanelQuanlyTaikhoan(Nhanvien nv) {
		...
		tfPassword = new CustomJTextField(nv.getTaikhoan().getMatkhau());
		tfPassword.setBounds(510, 82, 264, 25);
		add(tfPassword);

		tfUsername = new CustomJTextField(nv.getTaikhoan().getTendangnhap());
		tfUsername.setBounds(146, 82, 264, 25);
		tfUsername.setEditable(false);
		add(tfUsername);
...
//		reloadTable();
	...
	private Taikhoan getDataFromTextField() {
		...
		if (validInputTextField()) {
			...
			tk.setMatkhau(tfPassword.getText().trim());
